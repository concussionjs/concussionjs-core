...
module.exports.listen = function(server,id) {
    ...
    db = mydb;
      ...
      //console.log("in listen callback", JSON.stringify(doc));
		      server.listen(doc.port);
		      appLocation = doc.location;
		      appName = doc.name;
		      //console.log("after server.address().port: " + server.address().port + " doc.port:",doc.port);
	       });
  });
};
...
module.exports.serveStaticFilesNoWriteHead = function(req,res,next) {
  ...
  path.exists(filename, function(exists) {
    ...
    fs.readFile(filename, 'binary', function(err, file) 
          ...
          if (filename && (filename.search('/admin') > -1 || filename.search('.html') > -1) && uri.search('.js') == -1 && filename.search('.css') == -1 && filename.search('/index.html') == -1)
				{
